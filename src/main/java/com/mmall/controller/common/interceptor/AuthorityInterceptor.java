...
public class AuthorityInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object handler) throws Exception {
        ...
        //解析参数,具体的参数key以及value是什么，我们打印日志//多线程用buffer，其线程安全
        StringBuffer requestParamBuffer = new StringBuffer();
        Map paramMap = httpServletRequest.getParameterMap();
        //map.entrySet（）是将map里的每一个键值对取出来封装成一个Entry对象在存到一个Set里面。用来迭代
        Iterator it = paramMap.entrySet().iterator();
        while (it.hasNext()) {
            //Map 接口里面有个内部接口 Entry,返回就是Map.entrySet的元素
            // The <tt>Map.entrySet</tt> method returns a collection-view of the map, whose elements are of this class.
